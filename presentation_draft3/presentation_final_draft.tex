...
\item<6-> Use $f_{ode}(\mathbf{X}(t_i),t_i)$ to approximate $\mathbf{X}(\tau_1)$ to approximate $\mathbf{X}(\tau_2)$ etc.
...
\item<4-> Taylor series analogy: Local error  $~h^p$, global $~h^{p-1}$.
