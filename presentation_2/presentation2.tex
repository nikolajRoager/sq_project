...
\item <4-> Arbitrarily set $\frac{\mu_0}{4\pi}|\vec{m}|= \SI{0.155}{\tesla\per\cubic\meter}$ .

\item <4-> Protons with speed $\SI{30000}{\meter\per\second}$
...
\begin{frame}
\frametitle{Runge-Kutta Dormand Prince 4-5}
...
\item<2-> Should also decide to increase step size.
...
\begin{frame}
\frametitle{Butcher Tableu of Dormand Prince 4/5}
...
Boost Odeint Dormand-Prince, relative and absolute error $10^{-6}$.

\end{frame}


\begin{frame}
\frametitle{Does it work? Another curious result}
\begin{centering}
\only<1>{%

At most 62 points from $t=\SI{0}{\micro\second}$ to $t=\SI{600}{\micro\second}$, adaptive.
}
\only<2>{%
\noindent\includegraphics[width=\linewidth]{dipole_h.pdf}
